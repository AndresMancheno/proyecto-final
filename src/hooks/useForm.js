...
const useForm = ({
	...
	const [values, setValues] = useState(initialValues),
		...
		getPropertiesNameSchema = validationSchema
			? Object.getOwnPropertyNames(validationSchema)
			: [],
		...
		runValidationRules = (property, value) => {
			...
			if (validationSchema[property].isEmail) {
				...
				result = validateField({
					...
					shortValue:
						existMinRule &&
						(typeof existMinRule.message === 'function'
							? existMinRule.message(min)
							: existMinRule.message),
				});
			}
		...
		runValidationErrors = ({ property, value, allValues }) => {
			const getErrors = validateAllFields
				? validate(allValues)
		...
		runValidateAllFields = () => {
			...
			if (validationSchema) {
				result = getRulesSchema.reduce((acc, _, i) => {
					...
					const getValidationResult = runValidationRules(
						property,
						values[property]
					);
